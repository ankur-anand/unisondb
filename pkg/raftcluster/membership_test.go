...
func (n *testNotifier) OnChangeEvent(event MemberEvent, info MemberInformation) {
	n.mu.Lock()
...
func (n *testNotifier) OnEvent(eventName string, payload []byte) {}

func (n *testNotifier) hasEvent(event MemberEvent, node string) bool {
	n.mu.Lock()
	defer n.mu.Unlock()
	for i, ev := range n.events {
		if ev == event && n.infos[i].NodeName == node {
			return true
		}
	}
	return false
...
func TestMembershipLiveClusterJoinLeave(t *testing.T) {
	...
	for {
		if n1.hasEvent(MemberEventJoin, "node2") {
			break
		}
	...
	deadline = time.Now().Add(5 * time.Second)
	for {
		if n1.hasEvent(MemberEventLeave, "node2") {
			break
		}
		if time.Now().After(deadline) {
